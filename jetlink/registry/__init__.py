...
__all__ = ['CATALOG_URL', 'COMMIT_PATCH_URL', 'DEFAULT_BIG_MODEL_REF', 'DRIVING_MODELS_TREE_URL', 'LFS_ENDPOINTS', 'POINTER_URL',
           ...
           'StopFn', 'VerifyError', 'fetch_catalog', 'fetch_pointer', 'is_ref', 'is_sha256', 'lfs_download', 'lfs_resolve',
           'parse_catalog', 'parse_pointer_text']
...
class Registry:
  ...
  def catalog(self, refresh: bool = False, max_age: float = CATALOG_MAX_AGE, opener=None) -> dict:
    ...
    if refresh or raw is None or fetched_at is None or time.time() - fetched_at > max_age:
      try:
        fresh = fetch_catalogs(opener=opener)
